...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/bundle/agent-team-web-profile': { kind: 'indirect', reason: 'The private Web layer delegates model composition to the Host-side Agent Teams profile.' },
  ...
  'packages/client/ui-notifications': { kind: 'none', reason: 'Browser-side OS-notification renderer over the sessions list snapshot; writes only user preference fields through the settings wire.' },
  ...
  'packages/host/file-picker': { kind: 'none', reason: 'The GUI-host file-picking seam registers nothing model-facing.' },
  'packages/host/file-picker-native': { kind: 'none', reason: 'The GUI-host file-picking backend registers nothing model-facing.' },
