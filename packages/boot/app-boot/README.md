...
- **One rejection checkpoint.** `assertEntriesActivated` keeps the exact reasons it folds into the boot diagnostic visible through the next process rejection checkpoint, so `installFailLoud` coalesces Loader's duplicate notification while unrelated unhandled rejections remain fatal for as long as startup is in flight. `installFailLoud` returns a guard whose `seal` closes that window: once a launcher reaches readiness, a later rejection is reported on stderr and to the surface's `report` hook instead of exiting, because a long-running session must not die over a stray rejection from a detached plugin task, a stream or pty error, a storage write, or a transport teardown.
