...
`harnessAvailable()` is a two-stage `require.resolve` probe, not a single check against each harness's own wrapper package. Probing only the wrapper (`@deepseek-ai/dsh-subagent-codex`, `@deepseek-ai/dsh-subagent-claude-code`) is a false positive whenever the wrapper is declared somewhere in the module graph — a consumer's `devDependencies`, say — but the platform CLI it actually depends on failed to install or was pruned from a production install. `harnessCliResolvable()` instead (1) resolves the wrapper's own `package.json` from a resolver anchored at this package, then (2) resolves the wrapper's real CLI dependency (`@openai/codex` for `codex`; `@anthropic-ai/claude-agent-sdk` for `claude-code`) from a `require` anchored at that manifest, trying the CLI specifier both bare and as its `/package.json` subpath and accepting either — there is no single specifier form both CLI packages resolve through: `@openai/codex` has no `main`/`exports` field (only `bin`), so only `/package.json` resolves; `@anthropic-ai/claude-agent-sdk` declares an `exports` map with a `.` entry but no `./package.json` entry, so only the bare specifier resolves. Only when both stages succeed does the harness count as available. The result is cached per process: whether a package is installed does not change while a process is running, and a spawned probe would carry latency and side effects the disabled-expression evaluator (a synchronous `eval`) cannot await.
...
- **Stage 1 (the wrapper package itself) is anchored on this package's own dependency declaration** — `@deepseek-ai/dsh-subagent-codex` and `@deepseek-ai/dsh-subagent-claude-code` are declared as `optionalDependencies` of `@saturnai/dsh-model-router` (restored after a Mars r2 finding: a prior round removed them, leaving stage 1's resolution ambiguous in a production install — it resolved only by incidentally sitting in this checkout's `devDependencies`, which pnpm prunes from a production install). The packaged desktop app's `resources/app/node_modules` was inspected read-only for this round (`C:\Users\izzy\AppData\Local\Programs\@dshcodedesktop\resources\app\node_modules`): it is a single flat directory, not pnpm's isolated per-package store, so once this package ships in a build every package in that tree is reachable from every other package's resolver regardless of its own declared dependencies — `optionalDependencies` is still the correct declaration (it documents the real, intentional edge and keeps `pnpm install` in this monorepo, and any future isolated-install packaging path, correct), it is just not the only thing standing between stage 1 and a false negative in the flat layout this app currently ships with. Neither wrapper package was present in that tree at inspection time (the external-harnesses feature has not shipped yet), so the packaged-app case remains unobserved end-to-end; the flat-layout observation is evidence about the resolution mechanism, not a claim that the packaged toggle has been exercised.
