...
| [`claims`](claims/README.zh.md) | 强制执行的 workspace claims：记录哪个 agent 拥有哪个文件表面的、带 TTL 租约的持久账本 | 无服务 key |
| [`design-brain`](design-brain/README.zh.md) | 可选启用的 SaturnAI MCP 连接，具有持久偏好和经验证的 Host 工具可用性 | `ctx.designBrain` |
| [`done`](done/README.zh.md) | 按会话的 definition of done：`done` 投影、`done:policy` 提示词片段、`/done` 命令与 `set_definition_of_done` | 无服务 key |
| [`orchestrate`](orchestrate/README.zh.md) | 已记录的按会话多任务（始终编排）模式：`orchestrate` 投影、其策略提示词片段与 `/orchestrate` 命令 | 无服务 key |
