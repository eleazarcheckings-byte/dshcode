...
const requiredArtifacts = [
  'packages/saturn/agent-team/lib/index.js',
  'packages/saturn/agent-team/lib/typert.remote-client.js',
...
describe.skipIf(!requiredArtifacts)('Agent Teams built LIB service', () => {
  it('loads the Host service and its generated browser contribution under plain Node', async () => {
    const urls = {
      host: artifactUrl('packages/saturn/agent-team/lib/index.js'),
      remote: artifactUrl('packages/saturn/agent-team/lib/typert.remote-client.js'),
    }
    const script = `
    ...
    expect(output).toEqual({
      ...
      methods: [
        '@saturnai/dsh-agent-team#agentTeams/createTask',
        '@saturnai/dsh-agent-team#agentTeams/updateTask',
        '@saturnai/dsh-agent-team#agentTeams/view',
      ],
    })
  })
})
