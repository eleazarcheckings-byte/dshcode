/** Keyless local HTTP fixture: one address a test points a provider's `baseURL` at. */

import { createServer } from 'node:http'
import type { IncomingMessage, ServerResponse } from 'node:http'
import type { AddressInfo } from 'node:net'

/** One request the fixture recorded: method, path, headers of interest, and parsed (or raw) body. */
...
/**
 * Start a local HTTP server whose handler answers every request by path/method. Requests are
 * recorded before the handler runs, so a handler that never responds still leaves an observable record.
