...
import { TeamTaskId } from '@saturnai/dsh-agent-team'
import type { TeamMemberView } from '@saturnai/dsh-agent-team'
...
A teammate is isolated in a private git worktree of the current commit by default. Its edits are invisible here until you call merge_teammate, and yours are invisible to it. merge_teammate applies that teammate's whole diff or none of it, and refuses any file another member has claimed — read the refusal, resolve the ownership, then merge again. Merge before you rely on isolated work, and before the task that depends on it starts. Isolation "shared" is opt-in: it puts the teammate in the Lead working directory, where edits are immediately visible to every member. Use shared only when members must edit the same checkout; split write work into disjoint scopes, record expected write scopes on shared tasks, and use task dependencies when work must be ordered. Give every teammate a concrete deliverable, its allowed write scopes, relevant context, and a verification requirement. Shared-task write scopes describe planned work. When claim_scope is available, each writer must also acquire its own file claims before editing and release them after verification; a task assignment alone grants no file ownership. Never bypass a claim denial.
