...
describe('dsh-tool-team', () => {
  it('installs the complete scoped schema and shared-checkout policy for roots and teammates', async () => {
    ...
    expect(leadPrompt).toContain('when multi-task mode is ON, proactively delegate')
    expect(leadPrompt).toContain('When multi-task mode is OFF, work in one thread')
    expect(leadPrompt).toContain('a task assignment alone grants no file ownership')
