...
此表是内置默认值；已配置的 `toolAliases` 条目会替换该 Claude Code 名称对应的默认值，其余默认条目保持不变。`PreToolUse`／`PostToolUse` matcher 会同时针对原始 DSH 工具名称与代指它的每个 Claude Code 名称求值，因此 matcher 可以任一方式书写（`Bash|PowerShell` 与 `bash|pwsh` 都可用）。合成 payload 的 `tool_name` 会报告代指实际被调用的 DSH 工具的第一个（按表顺序）Claude Code 名称，若没有名称代指它——例如 `terminal_open`、某个 MCP 工具，或本表未命名的其他工具——则报告原始 DSH 名称。`tool_input` 始终是原样的 `exec.arguments`——从不重命名任何字段——对 `bash`、`pwsh`、`write`、`edit` 这四个调用而言，这意味着 `tool_input` 与其被报告所归属的 Claude Code 工具天然字段兼容：这四个 DSH 工具本身的字段命名（`command`、`file_path`、`content`、`old_string`、`new_string`）已与 Claude Code 自身的 `Bash`／`PowerShell`／`Write`／`Edit` schema 一致。这种兼容性**不会**扩展到上表其余每一个别名上：`str_replace_editor` 调用仍会被报告为 `tool_name: "Edit"`（或 `MultiEdit`／`NotebookEdit`），但保留其自身字段名（`path`、`file_text`、`old_str`、`new_str` 等）；`read_image` 调用会被报告为 `tool_name: "Read"`，同样保留自身字段——二者都不具备该 `tool_name` 下一个原生 Claude Code 钩子所期望的 `file_path`／`old_string`／`new_string` 形状。参见[已知限制](#known-limitations-and-deferred-work)。当 `toolAliases` 的值不是一个「每个值都是非空字符串数组」的对象时，插件加载时会拒绝该值。
...
- **`str_replace_editor` 与 `read_image` 调用的 `tool_input` 字段名与其所报告的 `tool_name` 不一致**——`toolAliases` 会把 `str_replace_editor` 调用报告为 `tool_name: "Edit"`／`"MultiEdit"`／`"NotebookEdit"`，把 `read_image` 调用报告为 `tool_name: "Read"`，与原生 Claude Code matcher 的预期一致，但 `tool_input` 从不被重新映射：`str_replace_editor` 自身字段是 `command`、`path`、`file_text`、`insert_line`、`new_str`、`old_str`（没有 `file_path`／`old_string`／`new_string`），`read_image` 也保留自身字段形状，而非 `Read` 的形状。为这两个 Claude Code 工具编写的钩子必须读取 `tool_input.file_path || tool_input.path`（正如 izzy 的 `protect-brakes.js` 已经做的那样），而不能只假设存在 `file_path`。只有 `bash`／`pwsh`／`write`／`edit` 才与其别名所报告的 Claude Code 工具真正字段兼容。
