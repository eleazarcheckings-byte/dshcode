...
This table is the built-in default; a configured `toolAliases` entry replaces the default for that Claude Code name, and every other default entry is kept. A `PreToolUse`/`PostToolUse` matcher is evaluated against both the raw DSH tool name and every Claude Code name aliasing it, so a matcher can be written either way (`Bash|PowerShell` and `bash|pwsh` both work). A synthesised payload's `tool_name` reports the first (table-order) Claude Code name aliasing the DSH tool that was actually called, or the raw DSH name when nothing aliases it — `terminal_open`, an MCP tool, or any other tool this table does not name. `tool_input` is always `exec.arguments` unchanged — no field is ever renamed — and for a `bash`, `pwsh`, `write`, or `edit` call that means `tool_input` is already field-compatible with the Claude Code tool it is reported under: those four DSH tools name their own fields (`command`, `file_path`, `content`, `old_string`, `new_string`) identically to Claude Code's own `Bash`/`PowerShell`/`Write`/`Edit` schemas. That compatibility does **not** extend to every aliased name in the table above: a `str_replace_editor` call is still reported under `tool_name: "Edit"` (or `MultiEdit`/`NotebookEdit`) but keeps its own field names (`path`, `file_text`, `old_str`, `new_str`, …), and a `read_image` call is reported under `tool_name: "Read"` with its own fields — neither carries the `file_path`/`old_string`/`new_string` shape a stock Claude Code hook for that `tool_name` expects. See [Known Limitations](#known-limitations-and-deferred-work). An invalid `toolAliases` value — anything other than an object whose every value is a non-empty array of non-empty strings — is rejected when the plugin loads.
...
- **A `str_replace_editor` or `read_image` call's `tool_input` field names don't match its reported `tool_name`** — `toolAliases` reports a `str_replace_editor` call under `tool_name: "Edit"`/`"MultiEdit"`/`"NotebookEdit"` and a `read_image` call under `tool_name: "Read"`, matching what a stock Claude Code matcher expects, but `tool_input` is never remapped: `str_replace_editor`'s own fields are `command`, `path`, `file_text`, `insert_line`, `new_str`, `old_str` (no `file_path`/`old_string`/`new_string`), and `read_image` carries its own field shape, not `Read`'s. A hook written for exactly those two Claude Code tools must read `tool_input.file_path || tool_input.path` (as izzy's `protect-brakes.js` already does) rather than assume `file_path` alone. Only `bash`/`pwsh`/`write`/`edit` are genuinely field-compatible with the Claude Code tool their alias reports.
