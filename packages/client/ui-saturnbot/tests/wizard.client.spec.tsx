...
describe('FirstRunWizard', () => {
  it('blocks Continue until the objective is filled, then persists only that field and advances', async () => {
    ...
    render(<FirstRunWizard snapshot={data} workspaces={[]} save={save} busy={false} t={t} onExit={vi.fn()} />)
  ...
  it('jumps directly to a step chosen from the step list', () => {
    const data = wizardSnapshot({ config: { ...config, goal: '', workspace: '', provider: '', model: '' } })
    render(<FirstRunWizard snapshot={data} workspaces={[]} save={vi.fn(async () => {})} busy={false} t={t} onExit={vi.fn()} />)
  ...
  it('generates connect forms from the catalog on the connections step', () => {
    ...
    render(<FirstRunWizard snapshot={data} workspaces={[]} save={vi.fn(async () => {})} busy={false} t={t} initialStep="connections" onExit={vi.fn()} />)
  ...
  it('finishes by saving the schedule fields and exiting the wizard', async () => {
    ...
    render(<FirstRunWizard snapshot={data} workspaces={[]} save={save} busy={false} t={t} initialStep="schedule" onExit={onExit} />)
  ...
  it('skips to advanced configuration without saving anything', () => {
    ...
    render(<FirstRunWizard snapshot={data} workspaces={[]} save={save} busy={false} t={t} onExit={onExit} />)
