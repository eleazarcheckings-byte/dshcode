...
The main harness shares one full-window canvas through `shell.background`, behind the application columns. The sky remains mounted across session selection and hero transitions. The hero supplies the planet's layout anchor; Saturn follows that anchor while it is present, and the sky continues when it is absent. The independent SaturnBot window keeps its own execution canvas and omits this decorative sky.

The monochrome starfield and shooting-star routes span the upper, middle, and lower window, on both sides. Faint constellation groups occupy the upper and lower areas. Slow changes in neutral light and star brightness provide ambience between shooting stars while star positions stay fixed. Only one shooting star appears at a time. Mouse proximity softly reveals constellation lines without moving the scene. A brief entrance fade respects the motion preference; smaller fields reduce density while retaining stars and constellations in the lower window.

An empty focused composer slows its ambient motion; a text or image draft, editing another text field, a hidden page, or an offscreen field stops it. Pausing holds the current ring orientation and reflection, including across focus changes. The global Motion control at the bottom right shares one preference with the sky and stores it in browser storage; the operating system's reduced-motion preference always wins. Full-window rendering uses a four-million backing-pixel budget, caps display density at 2, and paints at most 30 frames per second. An SVG supplies the static fallback when the canvas backend or visible dimensions are unavailable.

Localized Build, Explore, and Review starters fill the empty draft without submitting a turn or replacing an existing draft. The pricing chip uses a steady status lamp.
