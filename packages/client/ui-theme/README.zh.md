...
同一张表也是玻璃材质的唯一所有者：`--dsw-glass-surface`（主题的 layer-3 表面、alpha 为 0.72，在该表的两个主题块中按明暗分支）、`--dsw-glass-filter`（`blur(14px) saturate(140%)`）与 `--dsw-glass-elevation`（elevation 描边加两层柔光，与其他 elevation token 一样逐元素重新声明）。没有其他样式表声明它们。玻璃面板用一条规则采用该材质——`border: 0; background: var(--dsw-glass-surface); backdrop-filter: var(--dsw-glass-filter); box-shadow: var(--dsw-glass-elevation);`——`@media (prefers-reduced-transparency: reduce)` 与 `@supports not (backdrop-filter: blur(1px))` 一起把填充解析为其不透明的 layer-3 层级色、把 filter 解析为 `none`，同时保持 elevation 不变，因此要求减少透明度的读者失去的是半透明与模糊，而不是表面（[样式规则](../../../docs/web-styling.zh.md#glass-surfaces)）。玻璃样式表 spec 断言该约定、两条回退路径，以及没有任何包在缺少模糊的情况下采用玻璃填充。
