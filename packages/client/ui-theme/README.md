...
The same sheet is the sole owner of the glass material: `--dsw-glass-surface` (the theme's layer-3 surface at 0.72 alpha, branched light/dark in the sheet's two theme blocks), `--dsw-glass-filter` (`blur(14px) saturate(140%)`), and `--dsw-glass-elevation` (the elevation stroke plus two soft layers, re-declared per element like the other elevation tokens). No other sheet declares them. A glass panel takes the material in one rule — `border: 0; background: var(--dsw-glass-surface); backdrop-filter: var(--dsw-glass-filter); box-shadow: var(--dsw-glass-elevation);` — and `@media (prefers-reduced-transparency: reduce)` together with `@supports not (backdrop-filter: blur(1px))` resolves the fill to its opaque layer-3 step and the filter to `none` while leaving the elevation in place, so a reader who asked for less transparency loses the translucency and the blur, never the surface ([styling rules](../../../docs/web-styling.md#glass-surfaces)). The glass stylesheet spec asserts the contract, the fallbacks, and that no package takes a glass fill without the blur beside it.
