...
`dsh-client-ui-settings-models` 让用户配置提供方 API 密钥、编辑模型列表并声明自定义路由。页面将提供方、设置和凭据描述合并为一份共享快照。First Light 收集个人偏好，并提供经验证的 DeepSeek 设置或明确的其他提供方配置路径；版本化声明和条件式 DeepSeek 弹窗随后出现。Models 页脚也让用户在引导结束后管理可选的 Host 端 SaturnAI 设计工具连接。
...
First Light 验证官方 DeepSeek 连接，或让用户明确推迟模型设置。推迟路径在收据中标明模型设置已推迟，完成个人资料、语气、记忆和设置封存后打开「设置 → 模型」。后续 DeepSeek 提示也提供明确的其他提供方路径，不写入凭据或模型设置。Models 打开时，待处理的引导暂停。已有可用提供方时，条件式 DeepSeek 提示不渲染即完成。

### 可选设计工具

First Light 与 Models 的常驻页脚共享 [Host 管理的设计大脑连接器](../../saturn/design-brain/README.zh.md)。「连接工具」持久化启用选择并挂载实际 MCP 适配器。「已连接」表示监督器的当前实例已连接，且 Host 工具注册表包含 compose 和 review；仅浏览器端的端点探测不能解锁收据。设置完成后仍可刷新状态和断开连接；配置文件管理的条目显示其管理位置。跳过设置会清除托管连接的启用选择；独立配置条目保持不变。说明文字指出内置高品质指南无需此服务，远程评审使用智能体提供的需求和证据。
...
**运行时不变式：** 不发布伴生入口。提供方和设计连接状态来自各自有类型的 Host API；生成的 Remote 和插槽注册随所属插件一起释放。
