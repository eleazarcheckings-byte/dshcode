...
import { DesignBrainController } from './design-brain.ts'
import { DesignBrainCard } from './DesignBrainCard.tsx'
import type { DesignBrainInjected } from './DesignBrainCard.tsx'
import type {} from '@saturnai/dsh-design-brain/remote'
...
export function apply(ctx: ClientContext): void {
  ...
  const brain = new DesignBrainController(ctx.remote.designBrain, t)
  ctx.effect(() => () => { brain.dispose() }, 'ui-settings-models: design brain projection')
  ...
  const firstLightInjected = (): FirstLightInjected => ({
    ...
    verifyDesignBrain: brain.verify,
    declineDesignBrain: brain.decline,
    pickWorkspace: async () => {
      try {
  ...
  ctx.effect(() => {
    ...
    const disposers = [
      ...
      ctx.on('connection/reset', () => {
        refreshModels()
        if (brain.state.getSnapshot().snapshot !== null) void brain.refresh()
      }),
    ]
    return () => {
  ...
  ctx.slots.inject('settings.models.footer', () => ctx.slots.register({
    name: 'settings.models.footer', id: 'design-brain', order: 10,
    inject: (): DesignBrainInjected => ({
      hooks: { designBrain: brain.state }, refresh: brain.refresh, connect: brain.connect, disconnect: brain.disconnect, t,
    }),
  }, DesignBrainCard))
