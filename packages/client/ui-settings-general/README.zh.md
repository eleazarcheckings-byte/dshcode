...
当内置 `archive` 分区已注册时，导航隐藏社区的 `archive-manager` 分区。移除内置贡献后，社区入口重新显示；其他分区保持可用。
...
折叠后的 Settings 控件保留视觉隐藏的本地化文字，因此窄侧栏中仍有可访问名称。
...
设置打开时将焦点移至关闭控件；Tab 和 Shift+Tab 在可用控件之间循环；关闭时将焦点还给触发器。嵌套对话框保留各自的键盘处理。

引导账本按升序投影，每次只挂载一个步骤。注册方持有持久化完成状态、能力就绪状态、文案、变更操作与可见包装，因此独立注册的流程无法堆叠，外壳也不会成为第二个配置事实来源。可见步骤自行持有弹窗框架与应用根节点 `inert` 生命周期。

打开 Settings 会临时卸载引导，避免其遮罩和 `inert` 所有权阻挡所选设置页面。关闭 Settings 后继续下一个未完成步骤，不会将任何提供商标记为就绪。若步骤的草稿需要在卸载后保留，则必须在调用 `openSection` 前自行保存草稿或完成该步骤。
