...
在 Windows/Linux 上按 **Ctrl+K**，在 macOS 上按 **Cmd+K**，或使用侧边栏入口，即可搜索对话标题及工作区名称或路径。匹配使用浏览器现有元数据，排除已归档和空白对话，最多显示 30 个目的地；**开始新任务**操作匹配时会额外显示。搜索不发送远程请求或模型提示词。使用方向键选择、Enter 打开、Escape 关闭；关闭后焦点返回调用控件。选择工作区会在其中开始对话。其他已打开的模态工作流保留其键盘快捷键。
