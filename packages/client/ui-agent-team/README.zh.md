...
本包向 Web 会话页头添加 Agent Teams action，让用户检查当前 roster、管理共享任务板并导航到 teammate 会话。它通过生成的 `ctx.remote.agentTeams` contribution 读取权威 Team 状态，并让普通 child history 导航继续使用稳定的 addressed-subagent 路径。需要挂载 Agent Teams 层的 Web 组合选择本包：已发布的 `@saturnai/dsh-web-app` bundle 默认挂载它，`@saturnai/dsh-agent-team-web-profile` 则为 base-backed profile 挂载它。两者只能挂载其中一个，因为同时挂载会在 Loader 中重复声明同一个 entry id。这个浏览器 projection 不扩展稳定 API Proxy、不存储 Team 状态，也不注册面向模型的输入。
...
已发布的 `@saturnai/dsh-web-app` bundle 直接挂载本包；base-backed profile 则在 Host-side Agent Teams 层之后通过 [`@saturnai/dsh-agent-team-web-profile`](../../bundle/agent-team-web-profile/README.zh.md) 挂载它。不要同时挂载两者，也不要把 Web profile 层叠加在 `dsh-web-app` 之上。Web Client loader 挂载 `/client` export；root Host export 不执行行为，本包也没有用户配置字段。
...
Client export 挂载来自 [`@saturnai/dsh-agent-team/remote`](../../saturn/agent-team/README.zh.md) 的生成式 `ctx.remote.agentTeams` contribution，然后通过 Cordis effect 注册 locale dictionary 与一个 conversation-header slot。Dispose plugin fiber 会移除这两项 registration。
...
- [Agent Teams Web profile](../../bundle/agent-team-web-profile/README.zh.md)——挂载本 Client plugin 的源码 checkout bundle。
- [Agent Teams service](../../saturn/agent-team/README.zh.md)——权威 roster、task 与 Remote 行为。
