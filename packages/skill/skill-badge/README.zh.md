---
description: "随包附带的「powered by Saturn AI」徽章 skill，供启用、使用或排查该可选徽章提供方的用户与维护者阅读。"
...
agent（智能体）可以通过该内置提供方加载官方「powered by Saturn AI」徽章 skill（技能），并遵循其指令，给文档、pull request 以及其他用 DeepSeek Harness 生成的内容添加署名徽章。该提供方没有配置，随附 CLI（命令行界面）组合以禁用状态包含该插件，因此部署方需要显式启用。该 skill 同时提供 Markdown 片段和随包分发的 PNG，供无法可靠导入远程图片的系统使用。
...
启用插件即可让 `saturn-badge` skill 出现在会话 skill 目录中；随后模型可以像加载任何其他 skill 一样加载它，并遵循其指令添加「powered by Saturn AI」徽章。
...
启用后，`saturn-badge` 会出现在会话目录的可用 skill 中。该 skill 覆盖远程 Markdown 徽章（基于 Shields.io）和随包分发的 PNG 徽章资源，后者用于无法可靠获取远程图片的目标环境。

### 徽章 skill 提供什么

- **Markdown 片段。** 在文档、pull request 与 merge request 中嵌入官方徽章标记的指令。
- **随包分发的 PNG 资源。** `saturn-badge.png`（726×120 源图，按 121×20 渲染），在无法导入远程图片的环境中可用。

### 可观察的成功与失败

启用插件会使 `saturn-badge` 出现在目录中并可凭名称加载；禁用或省略该行则它不会出现在任何目录中。由于提供方不可变，发现始终成功且恰好返回一个 skill，绝不会报告部分结果。
...
该提供方是一个不可变、同步注册的 skill 来源：它以 `saturn-badge` 作为提供方名称、按内置 skill rank（600）注册一个固定候选项，把随包分发的 `assets/` 目录作为该 skill 的目录资源基底公开，并在每次加载时从随包分发的 `assets/saturn-badge.md` 文件读取 skill 正文。
...
| [`assets/`](assets/) | 随包分发的 skill 正文（`saturn-badge.md`）与 PNG 资源（`saturn-badge.png`） |
...
- **固定一个 skill，无运行时自定义**——提供方恰好贡献 `saturn-badge` 这一个 skill；需要其他徽章变体的部署请自行编写 skill。
