...
The reviewer subagent still needs a real desktop and 400px-wide screenshot (`skills/premium-web-experience/scripts/review-web.mjs`, above) — `review-grade.mjs` grades markup, not pixels, and is the other half of the loop, not a replacement for looking at the rendered page.
...
- **`review-grade.mjs` is a static probe, not a browser.** No layout engine: tap-target size is not measured at all. Rendered character measure (characters per line) is split across both scripts instead: `review-web.mjs` computes it from a real rendered page — median characters-per-line over the first 20 lines of the main prose container, sampled with `Range.getClientRects()`, reported as `{measure_ch, pass: <= 75}` on its `desktop` view — and `review-grade.mjs` reads that value from the `--report` file it is given. Supply no `--report` (or one whose `desktop` view carries no `measure`) and the `measure` field on the written rubric stays `UNVERIFIED`, never a guessed `PASS`; it is reported outside the fixed 12-item checklist array and does not gate `overallVerdict`, which stays scoped to those 12 criteria. `oklch()`/`lab()` color declarations are not resolved by `jsdom`'s CSSOM and are reported as unresolved, never guessed. JS-driven motion (anything not expressed as a CSS `animation`/`animation-timeline`) is invisible to it. Four of the twelve checklist items are always `UNVERIFIED` by design (see above) — the probe never substitutes a guess for the fresh-context reviewer's judgment on those.
