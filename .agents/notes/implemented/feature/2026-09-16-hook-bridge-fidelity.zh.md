...
两个新 spec 文件直接覆盖该层：`tool-aliases.spec.ts` 针对精确的默认表对 `reverseToolAliases`／`claudeToolName`／`toolMatchCandidates`／`validateToolAliases` 以及 `parseClaudeCodeConfig` 的不支持事件跳过做单元测试（全部为纯函数、不涉及子进程）；`tool-alias-bridge.spec.ts` 驱动真实 agent 循环 + 真实 bash 执行器 + 真实钩子 shell 脚本（既有套件"优先使用真实实现"的规则），证明 `Bash|PowerShell` matcher 会拒绝 `bash` 与 `pwsh` 这两个 DSH 工具调用、但不拒绝 `read`；`Edit|Write|MultiEdit` matcher 会拒绝 `edit`／`write`／`str_replace_editor`、但不拒绝 `grep`；捕获到的 `bash` 调用 `PreToolUse` payload 携带 `tool_name: "Bash"` 且 `tool_input.command` 等于命令文本；捕获到的 `PostToolUse` payload 携带 `tool_name: "Edit"` 且 `tool_response` 为工具的结果文本；已配置的 `toolAliases` 覆盖不影响其余默认值继续生效；格式错误的 `toolAliases` 值在加载时被拒绝（`ctx.plugin(...).rejects.toThrow`）；`ConfigChange`／`PreCompact` 都会产生一条指名警告。fix-round-1 新增的 `known-events.spec.ts` 修复了 Mars 在同一改动中发现的回归：settings 文件回退路径（`hooksMap = asObject(root.hooks) ?? root`）不得把普通 settings key（`model`、`permissions`、`env`）误判为不支持的 hook 事件，同时在同样的裸根形态下仍需标记真正的事件（`ConfigChange`、`PreCompact`）。`tsc -p packages/hooks/hooks-claude-code` 干净通过。

`tool-aliases.spec.ts`（10 个测试，纯函数、不涉及子进程）与 `known-events.spec.ts`（2 个测试，同类）与平台无关，本次会话中运行并通过：10/10 与 2/2。既有的 `config.spec.ts`（纯解析、不涉及子进程，12/12）同样通过，确认没有回归。`tool-alias-bridge.spec.ts` 用 node 的 `path.join` ——而非正斜杠——构建每一个钩子 `command` 字符串，这在本 win32 主机上会产生一个反斜杠路径，被 `bash -c "<command>"` 在命令行参数本身中打乱（`command not found`，退出码 127）；这是一个预先存在、已独立复现的环境限制（针对改动前的源码同样可复现），也正是 `vitest.config.ts` 已经在 win32 上排除 `packages/hooks/*` 的原因。本次会话中针对一个解除该排除项的、临时且未提交的 vitest 配置直接运行时，已提交的 `tool-alias-bridge.spec.ts` 在本机上测得 **6 个失败 | 4 个通过（共 10 个）**；每一处失败都可追溯到反斜杠路径被打乱（例如某个 `deny.sh` 命令被当作被打乱的 Windows 路径调用，或某个 `payload` 捕获文件从未被打乱的脚本写入），而非别名桥接的匹配／payload 逻辑本身——该逻辑已由通过的 `tool-aliases.spec.ts` 单元测试独立覆盖。TypeScript 与纯逻辑套件是本会话中已验证的证据；Linux CI 通道（未排除 `packages/hooks/*`）是 `tool-alias-bridge.spec.ts` 与既有子进程驱动套件（`bridge.spec.ts`／`coverage-*.spec.ts`，二者在本机上表现出同一类与本次改动无关的预先存在 win32 路径失败——仅 `bridge.spec.ts` 本身就在本机上失败 9/15，且未被本 cell 修改）的权威依据。
