...
`IShellItem.GetDisplayName` transfers a NUL-terminated UTF-16 allocation to the caller. `koffi.decode.string16(address)` reads the string at that address, and `CoTaskMemFree` runs in `finally`. An external ArrayBuffer from `koffi.view` is forbidden by [Electron’s memory cage](https://www.electronjs.org/blog/v8-memory-cage), including in `ELECTRON_RUN_AS_NODE` mode; this can terminate the worker before JavaScript reports an error ([issue #47](https://github.com/whitelonng/dshcode/issues/47)). A fixed-size memory view also cannot infer the allocation length and can read inaccessible pages; a byte-wise terminator check truncates valid code units whose low byte is zero, such as `开` (U+5F00). The COM fixture uses actual koffi decoding of an allocation containing only the path and terminator, checks bulk reads against that size, and covers decoding failure cleanup. The assembled Web workspace snapshot separately pins Chinese-path adoption and display.
...
- The desktop packaging workflow runs the binding regressions with real koffi decoding under Electron on macOS and Windows, verifies the test worker is actually running Electron, and on Windows also checks worker launch and opens/aborts the packaged picker. Completing a real Windows selection remains a manual check; open/abort coverage does not execute path decoding.
