...
`IShellItem.GetDisplayName` 将以 NUL 终止的 UTF-16 分配交给调用方。`koffi.decode.string16(address)` 读取该地址处的字符串，`CoTaskMemFree` 在 `finally` 中执行。[Electron 的内存笼](https://www.electronjs.org/blog/v8-memory-cage)禁止 `koffi.view` 创建外部 ArrayBuffer，`ELECTRON_RUN_AS_NODE` 模式也不例外；这可能在 JavaScript 上报错误前终止 worker（[issue #47](https://github.com/whitelonng/dshcode/issues/47)）。固定大小的内存视图还无法推断分配长度，可能读到不可访问的页面；逐字节检查终止符会截断低字节为零的有效代码单元，例如 `开`（U+5F00）。COM fixture 对仅容纳路径及终止符的分配调用真实 koffi 解码，按实际分配大小检查整块读取，并覆盖解码失败时的清理。组装后的 Web 工作区快照另行固定中文路径的接入与显示。
...
- 桌面打包工作流在 macOS 和 Windows 的 Electron 中运行使用真实 koffi 解码的绑定回归测试，确认测试 worker 确实运行在 Electron 中；Windows 还会检查 worker 启动，并打开和中止打包后的选择器。完成真实 Windows 目录选择仍需手动检查；打开／中止覆盖不会执行路径解码。
