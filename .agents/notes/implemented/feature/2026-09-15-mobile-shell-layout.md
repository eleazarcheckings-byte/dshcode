...
### `ui-conversation/skeleton/ConversationSession.tsx` — the header is a touch surface too

The first cut raised the strip, the drawer, the composer seat and the tool disclosure rows to 44px and stopped there, which left the row a phone user reaches for most often at its desktop size: the view tabs are a 13/16 label over an 11px pad (27px measured), and the `actions` / `utilities` seats — Agent Team, the job list, Definition of Done — render at 21px. Those seats are filled by packages the sheet does not own and must not have to change, so the header publishes one durable anchor, `data-conversation-header`, and the sheet raises every `button` inside it. The view tabs carry a second anchor, `data-conversation-tab`, for one reason: the selected state is a 2px bar drawn at the button's own floor, so a taller button has to keep its label sitting on that floor (`inline-flex` + `align-items: flex-end`) instead of floating in the middle of the new box. Measured in Chromium at 390×844 over the real stylesheets: 28/21/21/27/27px before, 44px on all five after, with `document.documentElement.scrollWidth` still 390.
...
Running vitest over `ui-layout`, `ui-sidebar`, `ui-theme` and `ui-conversation` — 73 files, 668 tests green. New: `drawer-trap.client.spec.ts` (Escape, both Tab wraps, the empty-panel fallback, both focus-timing arms, restore and its idempotence), `mobile-columns.client.spec.ts` (the breakpoint and the store's drawer semantics), `mobile-frame.client.spec.tsx` (the phone shape, the drawer's dialog markup, close on backdrop/Escape/route change, focus hand-back, and the desktop shape at 1280px), `keyboard-inset.client.spec.ts`, `mobile-styles.client.spec.ts` (the sheet contract above), `drawer-anchor.client.spec.tsx`. Three existing expectations were widened for the two new store fields, one for the new sheet in the mount order, and the sidebar snapshots re-recorded for the new root attribute.

`session-header-anchors.client.spec.tsx` holds the two header anchors from the DOM side (the header is the `<header>` element, every button of the row is inside it, each tab is marked and keeps `role="tab"`), and `mobile-header-targets.client.spec.ts` holds the sheet's side of the same contract (44px on both axes, the tab label pinned to its bar, attribute selectors only, still one breakpoint). Both fail against the sheet as it stood before this round.

`apps/web/tests/mobile-layout.e2e.ts` drives the real composition at 390×844: the drawer opens from the strip, sits at `left: 0` across 84% of the screen with a live backdrop, and closes on the route change when a session row is picked; no control under 44px anywhere in the phone chrome — the strip, the drawer, the conversation header and the composer seat, which is exactly the set the sheet governs; the composer lifts clear of a simulated 336px keyboard band without clipping; `document.documentElement.scrollWidth` stays ≤ 390 in both states. Screenshots go to `.artifacts/`, where every sibling scenario writes. The scenario cannot boot in the tree as it stands — `launchWebScaffold` throws `failed to apply loader entry saturn-tool-media … Cannot read properties of undefined (reading 'gemini')` from a stale `packages/saturn/tool-media/lib/index.js`, which predates this work and stops every web scenario, not this one — so it re-runs green only after a build refreshes that package's `lib/`.
