...
`dsh-llm-pi-ai` gains a generic `modelsEndpoint: true` flag on a declared route (`src/live-models.ts`): the route's models come from `GET {baseURL}/models`, served ten minutes after a successful listing (a failed listing is retried after 30 seconds, and a changed stored key refetches at once), merged after the route's own `models`, which act as the seed and the failure fallback. Entries with a `providers[]` array are kept only when a provider is `live`; context is the largest live `context_length`, tools are any live `supports_tools`, and image input only when `architecture.input_modalities` says so. The route accepts `org/name:<policy|provider>` and any typed `org/name` id. Router 401/402/403/404/429 answers become `[huggingface:<kind>]` failures; a 403 (a gated model, or a token missing the "Make calls to Inference Providers" permission) carries `ACCESS_DENIED` rather than `AUTH`, so no client reports it as an invalid key. Other `modelsEndpoint` routes get provider-neutral listing failures. The base bundle declares `huggingface` (`HF_TOKEN`, three seeds, `supportsDeveloperRole: false`).

The composer picker shows the Hugging Face group with a search field, context and tools badges, a typed model-id entry, and a Routing pane (fastest, cheapest, preferred, each live provider). Settings → Models renders a Hugging Face card: token onboarding with the fine-grained token link, a searchable live list with context and tools badges, a routing policy, pinning into the route's `models`, and localized router failures.
