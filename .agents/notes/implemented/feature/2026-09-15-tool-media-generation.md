...
Commit trail (disclosed for the RED→green audit): the package's `src/`, both READMEs, and six lines of the RED spec's mock HTTP handler landed in `78a06d26b1`, whose subject reads `docs(saturn): re-record translation-pairing sidecars for hygiene-docs READMEs`; the RED spec itself is `25cb088cf6`, and `git log --all` holds no `feat(` commit for tool-media. The spec edit changed only the mock (a queued Higgsfield job plus its status poll), not an assertion. Audit the chain against those two hashes; the [commit-type scope gate](../process/2026-09-16-commit-type-scope-gate.md) now refuses such a mislabel at commit time.
