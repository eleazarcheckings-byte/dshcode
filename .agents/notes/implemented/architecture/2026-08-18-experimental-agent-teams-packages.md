...
> Packaging update (2026-09-15): the Agent Teams Web profile layer was deleted. Its only job was to insert the `ui-agent-team` row for a Web composition that does not mount the shipped bundle, and `@deepseek-ai/dsh-web-app` declares that row itself, so the layer had become a duplicate-mount footgun with no remaining purpose. The Host-side Agent Teams profile layer below stays current for base-backed profiles.
...
`packages/experimental/agent-team`, `packages/experimental/tool-agent-team`, `packages/experimental/agent-team-profile`, `packages/experimental/client-ui-agent-team`, and the Agent Teams Web profile layer are private workspace packages. The [experimental package naming decision](2026-08-19-experimental-package-name-prefix.md) owns their npm names and promotion rename; this note owns their placement, release exclusion, and dependency isolation.
...
## Promotion outcome (2026-09-14)

All five packages left `packages/experimental/` and became release members of the `@saturnai` family: `packages/saturn/agent-team` (`@saturnai/dsh-agent-team`), `packages/saturn/tool-agent-team` (`@saturnai/dsh-tool-agent-team`), `packages/client/ui-agent-team` (`@saturnai/dsh-client-ui-agent-team`), `packages/bundle/agent-team-profile` (`@saturnai/dsh-agent-team-profile`), and the Agent Teams Web profile layer, which was deleted on 2026-09-15 (see the packaging update above). Each manifest dropped `private`, stated `publishConfig.access: public`, and recorded `repository.directory`; every version matched the existing `@saturnai` packages at `0.1.0`.

The three Team rows moved from the private profile layer to the shipped Web bundle patch, `packages/bundle/web-app/cordis.patch.yml`. That placement follows the invariant a deployment-side `duplicate loader entry id` incident established: a Saturn feature row is declared once, in the shipped web bundle patch, because the boot include flattens every layer into one entry list and the Loader rejects a repeated explicit id. The Web profile layer was retired once the shipped bundle owned the `ui-agent-team` row; the Host layer therefore stays available only for base-backed compositions that do not mount the shipped Web bundle's Team rows, and a stock Web profile adds neither.

Two release-infra gaps remain outside this note: the release family still enumerates members by npm-scope prefix while the release-family declaration admits only `@deepseek-ai/`, so the `@saturnai` family is not yet a publishing family; and no named stable owner has signed the promotion. The Web agent preset still mounts the legacy continuable-child controls inside its own scope, which a top-level Host override cannot disable, so both delegation surfaces can reach the model until the preset becomes Team-aware.
