...
All five packages left `packages/experimental/` and became release members of the `@saturnai` family: `packages/saturn/agent-team` (`@saturnai/dsh-agent-team`), `packages/saturn/tool-agent-team` (`@saturnai/dsh-tool-agent-team`), `packages/client/ui-agent-team` (`@saturnai/dsh-client-ui-agent-team`), `packages/bundle/agent-team-profile` (`@saturnai/dsh-agent-team-profile`), and `packages/bundle/agent-team-web-profile` (`@saturnai/dsh-agent-team-web-profile`). Each manifest drops `private`, states `publishConfig.access: public`, and records `repository.directory`; every version matches the existing `@saturnai` packages at `0.1.0`.

The three Team rows moved from the private profile layer to the shipped Web bundle patch, `packages/bundle/web-app/cordis.patch.yml`. That placement follows the invariant a deployment-side `duplicate loader entry id` incident established: a Saturn feature row is declared once, in the shipped web bundle patch, because the boot include flattens every layer into one entry list and the Loader rejects a repeated explicit id. The two profile layers therefore stay available only for compositions that do not already mount the shipped Web bundle's Team rows, and a stock Web profile adds neither.
