...
Windows native-directory-picker workers spawn the packaged executable with `ELECTRON_RUN_AS_NODE=1` in that controlled child's environment. Electron therefore executes the worker entry as Node instead of relaunching DSHCode and losing the single-instance lock; the parent application remains in normal Electron mode, and users do not configure this variable.

The CLI manifest explicitly exports `./profile-boot` and `./package.json`. TypeScript path mappings cannot prove installed ESM resolution: a built file without its package subpath export still fails before Electron reaches application startup. Each native packaging job launches the real main entry and waits for the rendered workspace interface with isolated application data before permitting installer upload; the picker-only smoke cannot cover this startup path.

The desktop suppresses the Web profile’s external-browser handoff with `--no-open` and uses the connection service’s authenticated loopback URL for its BrowserWindow. The profile launcher respects `watchUserPatches: false` even for a live-reload profile, so its config-only HMR fallback is never mounted by the desktop.

## Verification

The desktop lifecycle suite pins loopback/port-zero arguments, activated-address validation, navigation policy, packaged Electron's missing main-module argument, and coalesced shutdown ordering. The runtime-closure gate covers the installed workspace peer graph. A production stage smoke boots the real Web profile, receives HTTP 200 on the OS-assigned loopback port, disposes it, and observes that the port no longer accepts connections; a native Electron launch exercises the same stage and window. The Windows packaging job also launches the branded unpacked executable and requires its real directory-picker worker to reach and abort the native dialog, pinning the child-only Node mode. Platform CI builds each installer on its target operating system, and the tag workflow proves release publication only after every matrix job succeeds. The renderer, model-visible inputs, and transcript output do not change, so existing Web snapshots remain the assembled-application coverage rather than gaining a duplicate desktop transcript.
