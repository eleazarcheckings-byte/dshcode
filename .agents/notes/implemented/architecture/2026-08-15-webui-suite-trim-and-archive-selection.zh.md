...
**管理页面共存。** Settings 的拥有者在内置 `plugins` 标签页存在时隐藏 `family-plugins`，在内置 `archive` 分区存在时隐藏 `archive-manager`。实时 slot 注册表使该规则不依赖加载顺序，并在内置贡献卸载后恢复社区入口。精确 id 避免依赖语言的文案匹配，保留插件的其他功能。归档列表优先复用侧边栏的实时或经过日志身份校验的标题投影缓存，仅为缺失标题读取日志并折叠；已缓存的空标题无需折叠。这使检查点时效语义与侧边栏一致，避免每次访问都读取全部归档日志，空归档不读取存储。原有搜索与批量操作保留；首次列表请求失败后可重试。
