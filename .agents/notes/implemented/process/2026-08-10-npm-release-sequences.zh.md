...
vendored 框架与 native 包发到 npmjs.com 的 `@deepseek-ai` scope；dsh 族发到 `@deepseek-ai` 和 `@saturnai` 两个 scope——从 `packages/experimental/` 提升上来的 Saturn 层自带一个 scope，而两个 harness 成员 `@deepseek-ai/dsh-base` 与 `@deepseek-ai/dsh-web-app` 依赖其中的包，只认 harness 那一个 scope 的发布会产出无法解析的依赖树。每个 family 在 [`scripts/release/families.ts`](../../../../scripts/release/families.ts) 里各自声明自己的 scope 集合，并拒绝集合之外的成员，这正是让 glob 与发布集不会各走各路的机制。每个发布成员都以 `public` 发布：[`scripts/check-workspace-constraints.ts`](../../../../scripts/check-workspace-constraints.ts) 会拒绝 `publishConfig.access` 不是 `public` 的发布成员。vendored 框架与 native 包先到这一级别，因为公开包装不上受限依赖（[理由](2026-08-13-public-vendor-and-native-sequences.zh.md)）。没有任何发布路径传 `--access`——该选项会覆盖真正拥有该级别的 manifest。
