...
The vendored framework and the native packages publish to the `@deepseek-ai` scope on npmjs.com; the dsh family publishes to `@deepseek-ai` and `@saturnai`, because the Saturn layer promoted out of `packages/experimental/` carries its own scope and two harness members — `@deepseek-ai/dsh-base` and `@deepseek-ai/dsh-web-app` — install packages from it, so a release naming only the harness scope would publish trees that cannot resolve. Each family declares its own scope set in [`scripts/release/families.ts`](../../../../scripts/release/families.ts) and refuses a member outside it, which is what keeps the glob and the publish set from drifting apart. Every release member publishes at `public`: [`scripts/check-workspace-constraints.ts`](../../../../scripts/check-workspace-constraints.ts) rejects a release member whose `publishConfig.access` is anything else. The vendored framework and the native packages reached that level first, because a public package cannot install a restricted dependency ([rationale](2026-08-13-public-vendor-and-native-sequences.md)). No publish path passes `--access`, because the flag would override the manifest that owns the level.
