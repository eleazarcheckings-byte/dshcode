...
Saturn's multi-task switch defaults to ON while Team tool guidance requires an explicit user request. The built-in presets also contribute legacy controls with names owned by Team tools. File claims deny overlapping acquisition but leave a peer able to write directly or release another session's claim.
...
First-party writes serialize within one workspace, including independent files. This favors predictable ownership over maximizing small-file write throughput. Shell tools, formatters, code generators and external editors still require coordination. Separate session workspace roots have separate ledgers, and external symlink changes during dispatch remain outside the ownership protocol.
