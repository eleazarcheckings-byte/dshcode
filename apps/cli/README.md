...
Native product shells import `runProfile` through `@deepseek-ai/dsh/profile-boot`; the package exports its built JavaScript entry and TypeScript declarations. `@deepseek-ai/dsh/package.json` remains available to SDK launchers that locate the installed CLI.
