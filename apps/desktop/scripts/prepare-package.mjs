...
function runPnpm(args) {
  ...
  if (entrypoint === undefined || entrypoint === '') {
    throw new Error('Saturn AI packaging must run through a pnpm package script.')
  }
 ...
 * the package-name prefix but is not a skin). Every scope that ships a
 * `dsh-client-ui-skin-*` package is scanned, not only the upstream one:
 * this product's own skin is `@saturnai/dsh-client-ui-skin-saturn`, so a
 * single-scope scan listed every skin in the picker except its own.
...
function assembleSkinsExtras() {
  const scopes = [
    { store: '@linxin666+dsh-client-ui-skin-', packageDir: '@linxin666' },
    { store: '@saturnai+dsh-client-ui-skin-', packageDir: '@saturnai' },
  ]
  ...
  let staged = 0
  for (const { store, packageDir } of scopes) {
    for (const entry of readdirSync(pnpmDir)) {
      if (!entry.startsWith(store)) continue
      const rest = entry.slice(store.length)
      const at = rest.indexOf('@')
      const id = at === -1 ? rest : rest.slice(0, at)
      const target = join(pnpmDir, entry, 'node_modules', packageDir, `dsh-client-ui-skin-${id}`)
...
export function preparePackage() {
  ...
  console.log(`Saturn AI packaging: ${skinCount} skin(s) staged at skins-extras`)
  const compatDir = assembleCompatExtras()
  console.log(`Saturn AI packaging: compat shim staged at ${compatDir ?? '(not installed)'}`)
  copyFileSync(resolve(appRoot, 'electron-builder.yml'), resolve(stageRoot, 'electron-builder.yml'))
  // The macOS target converts the SVG master; the Windows target takes the
  // committed multi-resolution ICO, which carries one optically compensated
  // raster per small size. Both are generated by the repo-level
  // scripts/icons/build-icons.mjs (apps/desktop/scripts/make-icon.mjs is the
  // older generator and no longer produces the full set).
  copyFileSync(resolve(appRoot, 'assets/icon.svg'), resolve(buildRoot, 'icon.svg'))
  copyFileSync(resolve(appRoot, 'assets/icon.ico'), resolve(buildRoot, 'icon.ico'))
  // NSIS artwork the `nsis` block names explicitly. Same committed assets, and
  // the same build/ destination electron-builder already resolves icon.ico
  // from, so the installer and uninstaller carry the ring mark instead of the
  // electron-builder defaults. Generated by the repo-level
  // scripts/icons/build-icons.mjs, which owns the optical tier ladder.
  for (const name of ['installerIcon.ico', 'uninstallerIcon.ico', 'installerHeaderIcon.ico']) {
    copyFileSync(resolve(appRoot, 'assets', name), resolve(buildRoot, name))
  }
