...
export function realizeSeedFixture(scaffold: WebScaffold, fixtureText: string, id: string): string {
  // Windows paths carry backslashes, which are not legal raw inside a JSON
  // string; substitute the JSON-escaped form so the realized fixture parses
  // on every platform.
...
function normalizeAria(snapshot: string, workspaceCwd: string, age: boolean): string {
  ...
  // path, so both spellings must collapse to the token. Split on both
  // separators: Windows paths carry backslashes, and skill content mixes
  // the two after the workspace dir.
